...
import { homepageFF, animepageFF, foryouFF, searchFF, detailAndAllEpisodeFF } from '../lib/freereels.js';
...
// --- FREE REELS ROUTE ---
router.get('/freereels/homepage', async (req, res) => {
    console.log('[FreeReels] Success Get Homepage');
    handleRequest(() => homepageFF(), req, res);
});

router.get('/freereels/animepage', async (req, res) => {
    console.log('[FreeReels] Success Get Animepage');
    handleRequest(() => animepageFF(), req, res);
});

router.get('/freereels/foryou', async (req, res) => {
    console.log('[FreeReels] Success Get Foryou');
    handleRequest(() => foryouFF(), req, res);
});

router.get('/freereels/search', async (req, res) => {
    let { query } = req.query;
    if (!query) {
        return res.status(400).json({ error: 'Parameter query dibutuhkan' });
    }
    console.log('[FreeReels] Success Get Search');
    handleRequest(() => searchFF(query), req, res);
});

router.get('/freereels/detailandallepisode', async (req, res) => {
    let { id } = req.query;
    if (!id) {
        return res.status(400).json({ error: 'Parameter id dibutuhkan' });
    }
    console.log('[FreeReels] Success Get Detail and All Episode');
    handleRequest(() => detailAndAllEpisodeFF(id), req, res);
});
